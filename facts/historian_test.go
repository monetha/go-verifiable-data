...
func TestHistorian_FilterChanges(t *testing.T) {
	...
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.TxData, FactProvider: factProviderAddress, Key: key})
	...
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.Bytes, FactProvider: factProviderAddress, Key: key})
	if err := provider.WriteString(ctx, passportAddress, key, "test only string"); err != nil {
		t.Errorf("Provider.WriteString() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.String, FactProvider: factProviderAddress, Key: key})
	...
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.Address, FactProvider: factProviderAddress, Key: key})
	if err := provider.WriteInt(ctx, passportAddress, key, big.NewInt(-123456)); err != nil {
		t.Errorf("Provider.WriteInt() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.Int, FactProvider: factProviderAddress, Key: key})
	if err := provider.WriteUint(ctx, passportAddress, key, big.NewInt(123456)); err != nil {
		t.Errorf("Provider.WriteUint() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.Uint, FactProvider: factProviderAddress, Key: key})
	if err := provider.WriteBool(ctx, passportAddress, key, true); err != nil {
		t.Errorf("Provider.WriteBool() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Updated, DataType: data.Bool, FactProvider: factProviderAddress, Key: key})

	// make some deletes
	if err := provider.DeleteTxData(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteTxData() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.TxData, FactProvider: factProviderAddress, Key: key})
	if err := provider.DeleteBytes(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteBytes() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.Bytes, FactProvider: factProviderAddress, Key: key})
	if err := provider.DeleteString(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteString() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.String, FactProvider: factProviderAddress, Key: key})
	if err := provider.DeleteAddress(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteAddress() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.Address, FactProvider: factProviderAddress, Key: key})
	if err := provider.DeleteInt(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteInt() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.Int, FactProvider: factProviderAddress, Key: key})
	if err := provider.DeleteUint(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteUint() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.Uint, FactProvider: factProviderAddress, Key: key})
	if err := provider.DeleteBool(ctx, passportAddress, key); err != nil {
		t.Errorf("Provider.DeleteBool() error = %v", err)
	}
	expectedChanges = append(expectedChanges, changeDetails{ChangeType: change.Deleted, DataType: data.Bool, FactProvider: factProviderAddress, Key: key})
	...
	t.Run("filter all order", func(t *testing.T) {
		testFilterChangesAll(t, hist, passportAddress, expectedChanges)
	})
}
...
func testFilterChangesAll(t *testing.T, hist *facts.Historian, passportAddress common.Address, expectedChanges []changeDetails) {
	...
	for _, ch := range changes {
		...
		actualChanges = append(actualChanges, actual)
	}
