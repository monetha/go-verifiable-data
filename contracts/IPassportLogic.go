...
// IPassportLogicContractBin is the compiled bytecode used for deploying new contracts.
const IPassportLogicContractBin = `0x`

// DeployIPassportLogicContract deploys a new Ethereum contract, binding an instance of IPassportLogicContract to it.
func DeployIPassportLogicContract(auth *bind.TransactOpts, backend bind.ContractBackend) (common.Address, *types.Transaction, *IPassportLogicContract, error) {
	parsed, err := abi.JSON(strings.NewReader(IPassportLogicContractABI))
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	address, tx, contract, err := bind.DeployContract(auth, parsed, common.FromHex(IPassportLogicContractBin), backend)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &IPassportLogicContract{IPassportLogicContractCaller: IPassportLogicContractCaller{contract: contract}, IPassportLogicContractTransactor: IPassportLogicContractTransactor{contract: contract}, IPassportLogicContractFilterer: IPassportLogicContractFilterer{contract: contract}}, nil
}
