...
func main() {
	...
	if *backendURL == "" {
		...
		monethaSession := eth.NewSession(contractBackend, monethaKey).SetGasPrice(gasPrice).SetLog(log.Warn)
		...
		passportOwnerSession := eth.NewSession(contractBackend, passportOwnerKey).SetGasPrice(gasPrice).SetLog(log.Warn)
