...
```bash
./bin/passport write-fact --factproviderkey fact_provider.key \
  --fkey monetha.jpg \
  --ftype txdata \
...
```bash
./bin/passport read-fact --out ./fact_image.jpg \
  --passaddr 0x1C3A76a9A27470657BcBE7BfB47820457E4DB682 \
...
```bash
./bin/passport read-fact --out ./ipfs_fact_image.jpg \
  --passaddr 0x1C3A76a9A27470657BcBE7BfB47820457E4DB682 \
  --factprovideraddr 0x5b2ae3b3a801469886bb8f5349fc3744caa6348d \
  --fkey monetha.jpg \
  --ftype ipfs \
...
Let's read what the value of the fact was during the first update. To do this, we need to run command `read-fact-tx`, specify the transaction hash `0xd43201d6b23a18b90a53bf7ef1fffad0b04af603c039b6617601a225a129c632` and the type of data `txdata`:

```bash
./bin/passport read-fact-tx --out monetha1.jpg \
  --passaddr 0x1C3A76a9A27470657BcBE7BfB47820457E4DB682 \
  --ftype txdata \
...
```bash
./bin/passport read-fact-tx --out monetha2.jpg \
  --passaddr 0x1C3A76a9A27470657BcBE7BfB47820457E4DB682 \
  --ftype ipfs \
...
```bash
./bin/passport read-fact-tx --out Monetha_WP.pdf \
  --passaddr 0x1C3A76a9A27470657BcBE7BfB47820457E4DB682 \
  --ftype ipfs \
...
If the fact provider specifies a `--datakey` parameter, the encryption key will be saved to a file, which will allow
...
* the address of the passport (`--passaddr` parameter)
...
* if the data is read by the fact provider, he need to specify data encryption key (`--datakey` parameter)
...
```bash
./bin/passport read-fact --out /dev/stdout \
  --passaddr 0x4026a67a2C4746b94F168bd4d082708f78d7b29f \
...
Now let's try to read the same private data as passport owner. Instead of parameter `--datakey data_enc.key`, parameter `--ownerkey pass_owner.key` is provided:
...
specify the data type `--ftype privatedata`, and specify either a data encryption key (`--datakey` parameter) or a
...
```bash
./bin/passport read-fact-tx --out /dev/stdout \
  --passaddr 0x4026a67a2C4746b94F168bd4d082708f78d7b29f \
...
Reading private data from the same transaction by the passport owner looks very similar (instead of `--datakey`
...
1. Now follow [Building the source](#building-the-source) steps to build the full suite of reputation SDK utilities,
   ...
   $ echo Johny | ./bin/passport write-fact --fkey name --ftype string \
        --factproviderkey ./data_provider.key \
        --passaddr 0x4AEb3678b689DbB3F502D927580f9829001C4BB6 \
        --backendurl http://localhost:22000
   $ ./bin/passport read-fact --out /dev/stdout --fkey name --ftype string \
